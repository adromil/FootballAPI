...
//	CGSize detailsLabelSize = [detailsLabel.text sizeWithFont:detailsLabel.font constrainedToSize:maxSize lineBreakMode:detailsLabel.lineBreakMode];
    CGRect detailTextRect = [detailsLabel.text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:detailsLabel.font} context:nil];
    CGSize detailsLabelSize = detailTextRect.size;
	totalSize.width = MAX(totalSize.width, ceil(detailsLabelSize.width));
	totalSize.height += ceil(detailsLabelSize.height);
	if (ceil(detailsLabelSize.height) > 0.f && (indicatorF.size.height > 0.f || labelSize.height > 0.f)) {
		totalSize.height += kPadding;
	}
